...
{
__shared__ float funct_vect[512][20];
...
//Here begins the calculation of main algorithmic loop
  ...
  for(int ii=0;ii<count;ii++){
   funct_vect[ty][ii]=v1.x*funct[index1+ii].coeff_x+v1.y*funct[index1+ii].coeff_y+funct[index1+ii].const_coeff;
  ...
  for(int ii=0;ii<count;ii++){
    v0.x+=d1[index+ii]*funct_vect[ty][ii];
    v0.y+=d1[index+ii+count]*funct_vect[ty][ii];
  }
