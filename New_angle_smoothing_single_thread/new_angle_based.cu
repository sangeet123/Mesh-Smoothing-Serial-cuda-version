...
fan_list[ty][i]=fan[index1+i];
//if(row==60)
//printf("%d\n",fan_list[ty][i]);
}
//perform smoothing
//count=fan_list[ty][0];
vr.x=d_vb[fan_list[ty][2]].x;
vr.y=d_vb[fan_list[ty][2]].y;
vr1.x=d_vb[row].x;
vr1.y=d_vb[row].y;
vr2.x=d_vb[fan_list[ty][count+1]].x;
vr2.y=d_vb[fan_list[ty][count+1]].y;
...
for(int j=2;j<count+2;j++){
      ...
      vr3.x=d_vb[fan_list[ty][index]].x;
      vr3.y=d_vb[fan_list[ty][index]].y;
...
//     if(row==104)printf("%f %f %f\n",temp0,temp1,temp2);
   ...
   temp[0][0]=temp[0][1]=temp[1][0]=temp[1][1]=0;
	...
	temp1=jacobian[index+2*k+1]=funct[index1+k].coeff_y;
        temp[1][1]+=powf(temp0,2);   
        temp[1][0]+=(temp0*temp1);
        temp[0][0]+=powf(temp1,2);
...
*/
 ...
 //  if(row==104)printf("%f %f %f %f\n",temp[0][0],temp[0][1],temp[1][0],temp[1][1]);
   ...
   for(int ii=0;ii<count;ii++){
    d1[index+ii]=temp[0][0]*jacobian[index+2*ii];
    d1[index+ii]+=(temp[0][1]*jacobian[index+2*ii+1]);
    d1[index+ii+count]=temp[1][0]*jacobian[index+2*ii];
    d1[index+ii+count]+=(temp[1][1]*jacobian[index+2*ii+1]);
